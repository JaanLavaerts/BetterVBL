...
import { StyleSheet, Text, View, ScrollView, TouchableOpacity, Image, Linking, Modal   } from 'react-native';
...
function Game(props) {
    ...
    <TouchableOpacity
        ...
        <View style={{flex:2,flexDirection:"row",justifyContent:'space-between'}}>
            ...
            <Text style={{fontWeight: 'bold'}}>{props.datum}</Text>
            </View>
            <View style={{flex:2,flexDirection:"row",justifyContent:'space-between'}}>
            <Text>{props.away}</Text>
            <Text style={{fontWeight: 'bold', color: '#0076ff'}}>{props.tijd}</Text>
        </View>
        <Modal visible={isModalVisible} animationType="slide" transparent={true}>
                    <TouchableOpacity activeOpacity={1} style={styles.centeredView} onPress={toggleModal}>
                        <View style={styles.modalView}>
                            <Text style={{fontSize: 30, fontWeight: 'bold', textAlign:'center', color: '#0076ff'}}>{props.datum}</Text>
                            <Text></Text>
                            <Text style={{padding: 2}}><Entypo name="trophy" size={14} color="black" /> {props.poule}</Text>
                            <Text style={{padding: 2, color: 'blue'}} onPress={() => Linking.openURL("https://google.com/maps/search/?api=1&query="+props.plaats)}><Entypo name="location" size={14} color="black" /> {props.plaats}</Text>
                            <Text></Text>
